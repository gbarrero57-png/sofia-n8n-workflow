...
const nodes = [
  ...
  // ── [6] Bot Pause Check ──────────────────────────────────────────────────
  {
    ...
    parameters: {
      jsCode: [
        'const ctx          = $input.first().json;',
        'const SUPA_URL = $env.N8N_HINODE_SUPABASE_URL || "https://mzprgmwcaegtkxmuzkrp.supabase.co";',
        ...
        '',
        '// 2. Upsert conversation — Prefer return=representation evita un GET extra',
        'let bot_paused = false;',
        'let conv_draft = null;',
        'try {',
        '  const upserted = await this.helpers.httpRequest({',
        '    method: "POST",',
        ...
        '    conv_draft = row.current_order_draft || null;',
        '  }',
        '} catch(e) {',
  ...
  // ── [8] Pre-Clasificador Keywords ────────────────────────────────────────
  {
    ...
    parameters: {
      jsCode: [
        ...
        '}',
        '',
        '// Escape del flujo ORDER: "cancelar", "salir", "empezar de nuevo"',
        'const cancel_kws = ["cancelar","salir","empezar de nuevo","reiniciar","olvida","olvidalo","no quiero"];',
        'if (cancel_kws.some(function(k) { return norm.includes(k); })) {',
        '  return [{ json: { ...$json, intent: "CANCEL_ORDER", skip_ai: true, classified_by: "KEYWORD", cancel_order: true } }];',
        '}',
        '',
        ...
        '}',
        '',
        '// ACK / THANKS — respuesta simple sin escalar a humano',
        'const ack_kws = ["gracias","muchas gracias","ok","dale","listo","entendido","perfecto","genial","excelente","de acuerdo","si","sí","claro","bueno"];',
        'if (ack_kws.some(function(k) { return norm === k || norm.startsWith(k + " "); })) {',
        '  return [{ json: { ...$json, intent: "GREETING", skip_ai: true, classified_by: "ACK_KEYWORD" } }];',
        '}',
        '',
        '// GREETING / MENU',
  ...
  // ── [10] Clasificador AI ─────────────────────────────────────────────────
  {
    ...
    parameters: {
      ...
      headerParameters: {
        parameters: [
          { name: 'Authorization', value: '={{ "Bearer " + $env.N8N_OPENAI_API_KEY }}' },
  ...
  // ── [16] Buscar y Preparar Productos ─────────────────────────────────────
  {
    ...
    parameters: {
      jsCode: [
        ...
        '',
        '// Filtro server-side por categoría — evita transferir catálogo completo',
        'let url = SUPA_URL + "/rest/v1/hinode_products?store_id=eq." + ctx.store_id + "&active=eq.true&select=name,description,category,price_sale,sku&order=price_sale.asc&limit=8";',
        'if (category_filter) url += "&category=eq." + category_filter;',
        'let products = [];',
        'try {',
        '  products = await this.helpers.httpRequest({ method: "GET", url,',
        '    headers: { apikey: SERVICE_KEY, Authorization: "Bearer " + SERVICE_KEY } });',
        '  if (!Array.isArray(products)) products = [];',
        '} catch(e) { console.warn("Products fetch failed:", e.message); }',
        '',
        '// Llamar OpenAI con contexto de productos',
        'const OPENAI_KEY = $env.N8N_OPENAI_API_KEY;',
  ...
  // ── [19] Gestionar Pedido (State Machine) ─────────────────────────────────
  {
    ...
    parameters: {
      jsCode: [
        ...
        '',
        '// ── Helper inline: guardar draft (sin .call — this.helpers disponible directo) ──',
        'const patchConv = async (body) => {',
        '  try {',
        '    await this.helpers.httpRequest({',
        '      method: "PATCH",',
        ...
        '      body, json: true',
        '    });',
        '  } catch(e) { console.warn("patchConv error:", e.message); }',
        '};',
        '',
        '// ── CANCEL: usuario quiere salir del flujo ORDER ──',
        'if (ctx.cancel_order === true) {',
        '  await patchConv({ current_order_draft: null, last_intent: "CANCEL_ORDER" });',
        '  return [{ json: { ...ctx, order_complete: false,',
        '    response_text: "\\u274C Pedido cancelado. \\uD83D\\uDE0A\\n\\n\\u00BFEn qu\\u00E9 m\\u00E1s puedo ayudarte?\\nEscribe *men\\u00FA* para ver el cat\\u00E1logo." } }];',
        '}',
        '',
        '// ── STEP 0: Sin draft → iniciar pedido ──',
        'if (!draft || !draft.step) {',
        '  await patchConv({ current_order_draft: { step: "awaiting_product", started_at: new Date().toISOString() }, last_intent: "ORDER" });',
        '  return [{ json: { ...ctx,',
        '    order_complete: false,',
        '    response_text: "\\uD83D\\uDE0A Vamos a armar tu pedido.\\n\\n" +',
        ...
        '}',
        '',
        '// ── STEP 1: Tiene producto → validar y pedir datos ──',
        'if (draft.step === "awaiting_product") {',
        '  if (!msg || msg.length < 3) {',
        '    return [{ json: { ...ctx, order_complete: false,',
        '      response_text: "Por favor dime qué producto deseas pedir \\uD83D\\uDE0A\\nEjemplo: *\\"1 Crema Corporal Acaí\\"*" } }];',
        '  }',
        '  await patchConv({ current_order_draft: { ...draft, step: "awaiting_info", product_desc: msg, updated_at: new Date().toISOString() }, last_intent: "ORDER" });',
        '  return [{ json: { ...ctx,',
        '    order_complete: false,',
        '    response_text: "\\u2705 Anotado: *" + msg + "*\\n\\n" +',
        '      "Ahora necesito tus datos de entrega en un solo mensaje:\\n" +',
        '      "\\uD83D\\uDC64 *Nombre — Dirección (distrito, ciudad)*\\n\\n" +',
        '      "Ejemplo: *\\"María García — Av. Javier Prado 123, Miraflores, Lima\\"*"',
        '  } }];',
        '}',
        '',
        ...
        '  if (!msg || msg.length < 5) {',
        '    return [{ json: { ...ctx, order_complete: false,',
        '      response_text: "Por favor envía tu *nombre y dirección* en un mensaje \\uD83D\\uDE0A\\nEjemplo: *\\"María García — Av. Lima 123, Miraflores\\"*" } }];',
        '  }',
        ...
        '  await patchConv({ current_order_draft: { ...draft, step: "awaiting_payment", customer_info: msg, updated_at: new Date().toISOString() }, last_intent: "ORDER" });',
        '  return [{ json: { ...ctx,',
        '    order_complete: false,',
        ...
        '      "Responde exactamente: *yape*, *plin*, *transferencia* o *efectivo*"',
        '  } }];',
        '}',
        '',
        '// ── STEP 3: Tiene pago → validar y crear pedido ──',
        'if (draft.step === "awaiting_payment") {',
        '  const msgL = msg.toLowerCase();',
        '  const payment_method = msgL.includes("yape") ? "yape"',
        '    : msgL.includes("plin") ? "plin"',
        '    : msgL.includes("transf") ? "transferencia"',
        '    : msgL.includes("efectivo") ? "efectivo"',
        '    : null;',
        '  if (!payment_method) {',
        '    return [{ json: { ...ctx, order_complete: false,',
        '      response_text: "No reconocí el método de pago. Por favor escribe exactamente: *yape*, *plin*, *transferencia* o *efectivo* \\uD83D\\uDE0A" } }];',
        '  }',
        '',
  ...
  // ── [24] Buscar Pedido ────────────────────────────────────────────────────
  {
    ...
    parameters: {
      jsCode: [
        ...
        'const store_id    = ctx.store_id;',
        '',
        '// Validar phone para evitar injection en query params',
        'const safe_phone = /^[+0-9]{7,15}$/.test(phone) ? phone : "";',
        'let orders = [];',
        'try {',
        '  orders = await this.helpers.httpRequest({',
        '    method: "GET",',
        '    url: SUPA_URL + "/rest/v1/hinode_orders?store_id=eq." + store_id + "&customer_phone=eq." + encodeURIComponent(safe_phone) + "&select=order_number,items,payment_status,delivery_status,created_at&order=created_at.desc&limit=3",',
        '    headers: { apikey: SERVICE_KEY, Authorization: "Bearer " + SERVICE_KEY }',
        '  });',
